...
const valueIcons = [FlaskConical, Leaf, Users, Heart];
  ...
  return (
    <div className="pt-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <AnimatedSection>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4">
              {about.pageHeroTitle}<span className="text-accent">{about.pageHeroHighlight}</span>
            </h1>
            <p className="text-white/70 text-lg max-w-2xl mx-auto">{about.pageHeroSubtitle}</p>
          </AnimatedSection>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <AnimatedSection delay={0.2} className="mt-10 space-y-5">
                <div className="flex items-start gap-4 p-5 rounded-2xl bg-primary-light border border-primary/10">
                  ...
                  <div>
                    <h3 className="font-semibold text-dark-navy mb-1">{about.mission}</h3>
                    <p className="text-gray-600 text-sm leading-relaxed">{about.pageMissionText}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-5 rounded-2xl bg-dark-navy border border-white/5">
                  ...
                  <div>
                    <h3 className="font-semibold text-white mb-1">{about.vision}</h3>
                    <p className="text-white/60 text-sm leading-relaxed">{about.pageVisionText}</p>
                  </div>
                </div>
              </AnimatedSection>
            </div>
            ...
            <AnimatedSection direction="right" delay={0.2}>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                {about.pageHighlights.map((item) => (
                  <div key={item} className="flex items-center gap-2 p-3 bg-white rounded-xl border border-gray-100 shadow-sm">
      ...
      <section className="section-padding bg-grey">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader
            badge={about.valuesBadge}
            title={about.valuesTitle}
            highlight={about.valuesHighlight}
          />
      ...
      <section className="section-padding bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader
            badge={about.journeyBadge}
            title={about.journeyTitle}
            highlight={about.journeyHighlight}
